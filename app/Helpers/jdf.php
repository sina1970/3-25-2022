...
class jdf{
    ...
    public static function convert_gerogian_to_array($date){
        $gDate = explode("-",$date);
        $gDate = array_map('intval', $gDate);
        $date=self::find_first_and_last_of_jalali_month(self::gregorian_to_jalali($gDate[0],$gDate[1],$gDate[2]));
        return $date;
    }
    public static function find_first_and_last_of_jalali_month($arrDate){
        $first_of_jalali_month = 1;
        $last_of_jalali_month = "";
        if(1<=$arrDate[1] && $arrDate[1]<=6){
            $last_of_jalali_month = 31;
        } elseif(7<=$arrDate[1] && $arrDate[1]<=11){
            $last_of_jalali_month = 30;
        } elseif($arrDate[1]==12){
            if (self::sLeapYear($arrDate[0])==true){
                $last_of_jalali_month = 30;
            }else{
                $last_of_jalali_month = 29;
            }
        }
        $begin_of_georgian_month = self::jalali_to_gregorian($arrDate[0],$arrDate[1],$first_of_jalali_month);
        $end_of_georgian_month = self::jalali_to_gregorian($arrDate[0],$arrDate[1],$last_of_jalali_month);
        $date = array("begin"=>$begin_of_georgian_month,"end"=>$end_of_georgian_month);
        return $date;
    }
